...
| `summary.fluency/accuracy/naturalness` | **必填**，0-10 数字（可含一位小数），驱动首页 4 维指标与 Profile 趋势图。**根据本次对话实际表现打分，禁止照抄示例值 7/6.5/6** |
| `summary.weak_areas` | 弱项标签（逗号分隔）——Profile 页弱项云。**根据真实错误归纳，禁止照抄示例「时态, 单复数」** |

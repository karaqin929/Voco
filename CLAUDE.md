...
## 版本机制（两个版本号，都是故意的）
- **`?v=NN`**（当前 v111）：HTTP/Cloudflare 缓存击穿。写在 index.html/sw.js 的资源 URL 上。
- **`voco-vNN`**（当前 voco-v121）：SW CacheStorage 名称——唯一能替换缓存中根文档 `/` 的手段（`/` 无法带 ?v=）。
...
- 当前线上：**v111 / voco-v121**（v105-v107 三连批 + v108/v110 熊条对齐两级修复 + v109 横幅/洞察双修复 + v111 全局解除截断，详见下）。

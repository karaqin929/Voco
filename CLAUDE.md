...
## 版本机制（两个版本号，都是故意的）
- **`?v=NN`**（当前 v104）：HTTP/Cloudflare 缓存击穿。写在 index.html/sw.js 的资源 URL 上。
- **`voco-vNN`**（当前 voco-v114）：SW CacheStorage 名称——唯一能替换缓存中根文档 `/` 的手段（`/` 无法带 ?v=）。
...
- 当前线上：**v104 / voco-v114**（v104 首页四维信息架构 + 任务 2 翻卡 + SM-2 纯库捞，详见下）。
- **v104 首页四维信息架构重构 + 任务 2 翻卡 + SM-2 断层修复（已部署 v104/voco-v114，2026-08-20 用户「先部署」确认）**：
  ...
  - ⚠️ **部署插曲（2026-08-20 下午，待明天收尾）**：线上 502/503 约 1h——已排查非文件/非代码问题（前端 13/13 齐全、v104 文件曾被健康服务过；FastAPI 纯 FileResponse 不可能被前端文件弄崩），日志显示「部署成功 → 200 OK → 优雅停机」= Render 免费实例睡眠/部署切换窗口。空提交 f92c941 重触发后 dashboard **Live @ 2b1d269**（线上确为 v104）。遗留：用户浏览器被旧 SW（voco-v113）锁住——待执行「无痕窗口验证 + DevTools Unregister SW + Clear site data + 硬刷新」，恢复后跑三模块线上验收。**教训**：免费实例睡眠 + SW 缓存锁会叠加出「部署成功但用户看不到新版」的假象；排查顺序应为 ① dashboard 状态 → ② 无痕窗口 → ③ 再怀疑代码。

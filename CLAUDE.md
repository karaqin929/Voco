...
## 版本机制（两个版本号，都是故意的）
- **`?v=NN`**（当前 v108）：HTTP/Cloudflare 缓存击穿。写在 index.html/sw.js 的资源 URL 上。
- **`voco-vNN`**（当前 voco-v118）：SW CacheStorage 名称——唯一能替换缓存中根文档 `/` 的手段（`/` 无法带 ?v=）。
...
- 当前线上：**v108 / voco-v118**（v105-v107 三连批 + v108 熊条左对齐修复，详见下）。

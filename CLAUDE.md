...
## 版本机制（两个版本号，都是故意的）
- **`?v=NN`**（当前 v121）：HTTP/Cloudflare 缓存击穿。写在 index.html/sw.js 的资源 URL 上。
- **`voco-vNN`**（当前 voco-v131）：SW CacheStorage 名称——唯一能替换缓存中根文档 `/` 的手段（`/` 无法带 ?v=）。
...
- 当前线上：**v121 / voco-v131**（v121 每日队列快照冻结 + 提示词模板词汇点/句型升级 + 教练契约「对话第一纠错第二」重构；v120 句型队列重排到期优先+新卡预算 10 + 错题防重复守卫 + 话题开场指令 + 不自然表达按根因组织；v119 小熊长按封杀全站扩展 + 体检工具自修三处 + 日期自动识别整体删除；v118 登录页熊头 PNG 化；v117 全盘审计修复十处；此前 v116 四合一 + v115 三合一及更早各批，详见下）。
